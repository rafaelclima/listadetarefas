	...
	if (window.innerWidth <= 768) {
		...
		if (btnExpandir.classList.contains("menu-aberto")) {
			...
			formulario.style.animation =
				"wipe-out-up 0.5s cubic-bezier(.25, 1, .30, 1) both";
			...
			// Menu está fechado
			menuLateral.style.animation =
				"circle-in-hesitate 0.8s cubic-bezier(.25, 1, .30, 1) both";
			formulario.style.animation = "";
			menuLateral.style.width = "100%";
...
function salvarCard() {
	...
	const prioridadeTarefa = document.querySelector(
...
function editarTarefa(btnEdit) {
	...
	cardContentDiv.forEach((cardsContent) => {
...
function concluirTarefa(concluirCard) {
  ...
  `;
	...
	const btnModalConcluirTarefa = divModal.querySelector(
		".btn.btn-danger.btn-excluir-tarefa",
	);
...
function excluirTarefa(excluirCard) {
  ...
  `;
	...
	const btnExcluirTarefa = divModal.querySelector(
		".btn.btn-danger.btn-excluir-tarefa",
	);
